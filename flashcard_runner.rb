...
#turn message - card x of 4, question
  def turn_message(round)
    puts "This is card number #{round.turns.length + 1} of #{@deck.count + round.turns.length}." #NEED TO FIX DECK count
...
new_game = FlashCardRun.new(first_round).start(first_round)
