...
RSpec.describe CardGenerator do
  ...
  it 'reads the cards file and creates Card instances for each' do
    ...
    expect(@cards.cards[10].question).to eq("What mountain's peak is furthest from the center of the Earth?")
    expect(@cards.cards[10].answer).to eq("Chimborazo")
    expect(@cards.cards[10].category).to eq(:Geography)
  end
end
