...
RSpec.describe CardGenerator do
  ...
  it 'converts each line of the text to an array' do
    filename = "./lib/cards.txt"
    gen = CardGenerator.new(filename)
    expect(gen.convert_to_array).to eq(["What is 5 + 5?,10,STEM", "What is Rachel's favorite animal?,red panda,Turing Staff", "What is Mike's middle name?,nobody knows,Turing Staff", "What cardboard cutout lives at Turing?,Justin bieber,PopCulture"])
  end

  it 'splits each string in the array at the , creating new arrays' do
    filename = "./lib/cards.txt"
    gen = CardGenerator.new(filename)
    cards_as_strings = gen.convert_to_array
