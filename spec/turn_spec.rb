require_relative '../lib/card' #links the card.rb file to the card_spec.rb file
require_relative '../lib/turn' #links the turn.rb file to the card_spec.rb file
...
RSpec.describe Turn do #rspec syntax to describe the Turn class
  ...
  it 'handles incorrect guesses' do #set up named 'handles incorrect guesses'
    incorrect_turn = Turn.new("Anchorage", @card) 
    #incorrect_turn is a new instance of the Turn class with the string "Anchorage"
    #inputs the string "Anchorage" into the Turn class
    #inputs the card instance into the Turn class
    #was incorrect because the answer is "Juneau"
    expect(incorrect_turn.correct?).to be false
    #execute expect the correct? method of the incorrect_turn instance to be false
    expect(incorrect_turn.feedback).to eq("Incorrect.")
    #execute expect the feedback method of the incorrect_turn instance to be equal to the string "Incorrect."
  end
end